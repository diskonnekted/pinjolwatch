    ...
    <div class="bg-slate-900 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.3)] border border-slate-800 overflow-hidden">
        @if($step == 0)
            <div class="p-10 text-center">
                <div class="w-20 h-20 bg-teal-900/30 rounded-2xl flex items-center justify-center text-teal-400 mx-auto mb-6 border border-teal-800/50">
                ...
                <div class="mt-10">
                    <button wire:click="startQuiz" class="px-10 py-4 bg-teal-600 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-teal-500 shadow-lg shadow-teal-900/20 transition-all active:scale-95">
                        Mulai Tes Sekarang
                    </button>
                </div>
        ...
        @elseif($step <= 10)
            <div class="p-10">
                <div class="flex justify-between items-center mb-8">
                    <span class="text-xs font-black uppercase tracking-widest text-teal-400 italic">Pertanyaan {{ $step }} / 10</span>
                    <div class="w-32 bg-slate-800 h-2 rounded-full overflow-hidden">
                        <div class="bg-teal-500 h-full transition-all duration-500" style="width: {{ ($step/10)*100 }}%"></div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-3">
                    @foreach([
                        ...
                        <button wire:click="answer({{ $score }})" class="group flex items-center justify-between p-5 border-2 border-slate-800 rounded-2xl hover:border-teal-500 hover:bg-teal-900/20 transition-all text-left">
                            <span class="font-bold text-slate-300 group-hover:text-teal-300">{{ $label }}</span>
                            <div class="w-6 h-6 rounded-full border-2 border-slate-700 group-hover:border-teal-500 flex items-center justify-center">
                                <div class="w-2.5 h-2.5 bg-teal-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            </div>
                        </button>
                    @endforeach
                </div>
        ...
        @elseif($step == 11)
            <div class="p-10 text-center animate-in fade-in zoom-in duration-700">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="{{ route('report') }}" class="px-8 py-4 bg-teal-600 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-teal-500 shadow-lg shadow-teal-900/20 transition-all">
                        Lapor Teror Pinjol
                    </a>
